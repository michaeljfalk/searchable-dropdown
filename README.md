...
No framework and zero runtime dependencies — nothing for **you** to compile. The
same prebuilt `dist/liveselect.js` + `.css` run in **plain HTML + vanilla JS,
Node/Express, EJS templates, and Blaze**. (The `dist/` files are generated from a
single source in `src/` by a dev-only esbuild step; see [Building from source](#building-from-source).)
...
**The library has zero runtime dependencies** and ships prebuilt — there is
nothing for you to compile. Reference the files directly:
...
The control has **one source of truth**, `src/liveselect.js` (a real ES module).
The two `dist/` entry points are generated from it by a dev-only [esbuild](https://esbuild.github.io/)
step — there is no hand-maintained second copy of the class:

```bash
npm install      # installs esbuild (a devDependency only)
npm run build    # src/liveselect.js → dist/liveselect.mjs (ESM) + dist/liveselect.js (UMD)
```

- `dist/liveselect.mjs` is a **genuine ES module** — it `export default`s the class
  directly, so bundlers (rspack/webpack/Vite/Meteor) and browser-native `import`
  get the real constructor, never `undefined`.
- `dist/liveselect.js` is **UMD** — the same file works as a `<script>` tag
  (`window.LiveSelect`) and via `require()`.

The shipped library still has **zero runtime dependencies**; esbuild is build-time
only. The built `dist/` files are committed and published, and CI fails if they
drift from `src/`. `dist/liveselect.css` and `dist/liveselect-auto.js` are
hand-authored static assets (the auto-mounter is a `<script>`-tag helper that
reads `window.LiveSelect`, so load `liveselect.js` before it).
...
```
src/
  liveselect.js        # SINGLE SOURCE OF TRUTH for the control (ES module)
scripts/
  build.mjs            # esbuild: src/ → dist/ UMD + ESM (dev-only)
dist/                  # generated by `npm run build`; committed + published
  liveselect.js        # UMD core (script tag / require) — built
  liveselect.mjs       # real ES module, `export default` the class — built
  liveselect.css       # themeable styles (hand-authored)
  liveselect-auto.js   # optional declarative data-* auto-mount (script-tag only)
server/
